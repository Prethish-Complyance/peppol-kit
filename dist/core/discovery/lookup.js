 ...
 */
export async function participantLookup(participantID, sml) {
    const start = performance.now();
    if (!participantID) {
        console.error('[Error]: You need to provide a valid Participant ID ("0235:1234567890") to lookup.');
        return;
    }
    let normalizedparticipantID = participantID.toLowerCase();
    ...
    const serviceGroupReferencesUrls = servicegroupreferences.map((element) => element.attributes?.href).filter((url) => Boolean(url));
    const serviceInformations = await Promise.all(serviceGroupReferencesUrls.map(async (url) => {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`SMP request failed: ${response.status} ${response.statusText}`);
        }
        return parseXml(await response.text());
    }));
    const information = await Promise.all(serviceInformations.map((ele) => parseInformation(ele)));
...
async function resolveNaptrRecord(normalizedparticipantID, sml) {
    ...
    try {
        ...
        throw new Error(`Participant ID is not registered in ${sml} SML`);
    }
}
async function smpLookup(smpUrl, normalizedparticipantID) {
    try {
        if (!smpUrl) {
...
async function parseInformation(ele) {
    const [documentIdentifierElement] = findElements(ele, 'id:DocumentIdentifier');
    const [endpointReferenceElement] = findElements(ele, 'wsa:Address');
    const [apCertificateElement] = findElements(ele, 'smp:Certificate');
    const [ProcessIdentifierElement] = findElements(ele, 'id:ProcessIdentifier');
    const [smpCertificateElement] = findElements(ele, 'X509Certificate');
    const [serviceDescriptionElement] = findElements(ele, 'smp:ServiceDescription');
    const [technicalContactUrlElement] = findElements(ele, 'smp:TechnicalContactUrl');
...
function parseCertificateInformation(certificateBase64) {
